...
void USART2_SendString(char* str)
{
    while (*str)
    {
        while (!(USART2->ISR & USART_ISR_TXE)); // Wait until TXE (Transmit Data Register Empty) flag is set
        USART2->TDR = *str++; // Write the data to transmit into the USART Data Register
    }
}
